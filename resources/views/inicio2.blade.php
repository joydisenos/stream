        ...
        @foreach ($camaras->chunk(3) as $row)
          <div class="row">
            @foreach ($row as $camara)
                <div class="col-md-4 col-md-offset-0 pad">
                  <a href="{{ url('/camara/'. hashid()->encode($camara->id).'?room='. hashid()->encode($camara->id)) }}">
                    ...
                    <div class="titulo-img">{{ title_case($camara->name) }} <span class="fav">Favoritos:$</span></div>
                    </div>
